...
def evaluate(model, data_loader, device,metric_collection,num_classes=2,criterion=None,scaler=None):
    ...
    for  images, labels in data_loader:
        ...
        labels = labels.detach().cpu()
        metric_collection.update(pred, labels)
        rvd_eval.update(pred, labels)
        for i in range(output.shape[0]):
            hd = hausdorff_distance(pred[i].numpy(), labels[i].numpy(),method='modified')
            hausdorff_distance_list.append(hd)
            #hd = directed_hausdorff(pred[i].numpy(), labels[i].numpy())
            rvd=ravd(pred[i].numpy(),labels[i].numpy())
            rvd_list.append(rvd)

    #print(hausdorff_distance_list)
    ...
    a=rvd_eval.get_score()
    return total_loss,np.mean(hausdorff_distance_list),a#np.mean(rvd_list)  # ,val_pred,val_label
