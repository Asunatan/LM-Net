...
class NeighborhoodAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if H < self.kernel_size or W < self.kernel_size:
            ...
            _, H, W, _ = x.shape
        #B,H,W,num_heads*C ->B, H, W, 3, num_heads,head_dim ->3,B,num_heads,H, W,head_dim
        ...
        x = NATTENAVFunction.apply(attn, v)#B,num_heads,H, W,head_dim
        x = x.permute(0, 2, 3, 1, 4).reshape(B, H, W, C)
        if pad_r or pad_b:
            x = x[:, :Hp, :Wp, :]
        x=self.proj_drop(self.proj(x)).contiguous()#.permute(0, 3, 1, 2).contiguous()

        return x


class NEWNeighborhoodAttention(nn.Module):
    """
    Neighborhood Attention Module
    """
    def __init__(self, dim, kernel_size, num_heads,
    ...
    def forward(self, xq,xkv):
        ...
        if H < self.kernel_size or W < self.kernel_size:
            ...
            pad_r = max(0, self.kernel_size - W)
            pad_b = max(0, self.kernel_size - H)
            xq = pad(xq, (0, 0, pad_l, pad_r, pad_t, pad_b))
            _, H, W, _ = xq.shape
        #B,H,W,num_heads*C ->B, H, W, 3, num_heads,head_dim ->3,B,num_heads,H, W,head_dim
        q=self.q(xq).reshape(B, H, W, self.num_heads, self.head_dim).permute( 0, 3, 1, 2, 4)
        kv=self.kv(xkv).reshape(B, H, W, 2, self.num_heads, self.head_dim).permute(3, 0, 4, 1, 2, 5)
