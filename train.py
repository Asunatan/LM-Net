...
def main_single(rank, k_fold, args):
    ...
    if args.test:
        ...
        print(" test_loss:{:.4f} test_accuracy:{:.4f} test_precision:{:.4f} test_recall:{:.4f} test_specificity:{:.4f} test_dice:{:.4f} test_iou:{:.4f} test_big_iou:{:.4f} test_hd:{:.4f} test_rvd:{:.4f}" \
                .format(test_loss, test_accuracy, test_precision, test_recall,test_specificity, test_dice, test_iou,test_big_iou,test_hd,test_rvd))
        metric_list=[
                test_loss, test_accuracy, test_precision, test_recall, test_specificity, test_dice, test_iou,test_big_iou,test_hd,test_rvd
                ]
    ...
    for epoch in range(start_epoch,args.epochs):
        ...
        val_loss,val_hd = evaluate(model=model, data_loader=val_loader, criterion=criterion,
                            metric_collection=val_metric_collection,
                            device=rank, scaler=scaler)
        test_loss,test_hd = evaluate(model=model, data_loader=test_loader, criterion=criterion,
                            metric_collection=test_metric_collection,
                            device=rank, scaler=scaler)
        scheduler.step()
        train_accuracy, train_precision, train_recall, train_specificity,train_dice, train_iou,train_big_iou = train_metric_collection.compute().values()
        val_accuracy, val_precision, val_recall, val_specificity,val_dice, val_iou,val_big_iou = val_metric_collection.compute().values()
        test_accuracy, test_precision, test_recall, test_specificity, test_dice, test_iou,test_big_iou = test_metric_collection.compute().values()
        print(
            " train_loss:{:.4f} train_accuracy:{:.4f} train_precision:{:.4f} train_recall:{:.4f} train_specificity:{:.4f} train_dice:{:.4f} train_iou:{:.4f}  train_big_iou:{:.4f}"\
                .format(train_loss, train_accuracy, train_precision, train_recall,train_specificity, train_dice, train_iou,train_big_iou))
        print(
            " val_loss:{:.4f} val_accuracy:{:.4f} val_precision:{:.4f} val_recall:{:.4f} val_specificity:{:.4f} val_dice:{:.4f} val_iou:{:.4f} val_big_iou:{:.4f} val_hd:{:.4f}" \
                .format(val_loss, val_accuracy, val_precision, val_recall,val_specificity, val_dice, val_iou,val_big_iou,val_hd))
        print(
            " test_loss:{:.4f} test_accuracy:{:.4f} test_precision:{:.4f} test_recall:{:.4f} test_specificity:{:.4f} test_dice:{:.4f} test_iou:{:.4f} test_big_iou:{:.4f} test_hd:{:.4f}" \
                .format(test_loss, test_accuracy, test_precision, test_recall,test_specificity, test_dice, test_iou,test_big_iou,test_hd))
        metric_list=[train_loss, train_accuracy, train_precision, train_recall,train_specificity, train_dice, train_iou,train_big_iou,
                val_loss,val_accuracy, val_precision, val_recall,val_specificity, val_dice,val_iou,val_big_iou,val_hd,
                test_loss, test_accuracy, test_precision, test_recall, test_specificity, test_dice, test_iou,test_big_iou,test_hd
                ]
